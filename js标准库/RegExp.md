...
上面代码中，字符串`hello world`不包含字符`a`、`b`、`c`中的任一个，所以返回`true`；字符串`bbc`不包含`a`、`b`、`c`以外的字符，所以返回`false`
...
上面代码，当连子符（dash）不出现在方括号之中，就不具备简写的作用，只代表字面的含义，所以不匹配字符`b`，只有当连子号用在方括号之中，才表示连续的字符序列。
...
修饰符（modifier）表示模式非附加规则，放在正则模式的最尾部
