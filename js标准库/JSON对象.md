...
`JSON.stringify`方法还可以接受一个数组，作为第二个数组，制定需要转成字符串的属性
...
上面代码中，对象`o`一共会被`f`函数处理三次，最后那行是`JSON.stringify`的输出。第一次键名为空，键值是整个对象的`o`；第二次键名为`a`，键值是`{b: 1}`；第三次键名为`b`，键值为 1

递归处理中，第一次处理的对象，都是前一次返回的值
...
如果参数对象有自定义的`toJSON`，那么`JSON.stringify`会使用这个方法的返回值作为参数，而忽略元对象的其他属性
